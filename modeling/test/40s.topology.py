...
for p in map_p2c:
    ...
    if p in ["RPS9","RPS3","RPS27","RPS1","RPS20","RPS5","RPS11"]:
       ...
       pdbbits+=simo.autobuild_model(p,datadir+'40S_43S_map_fitted_helix_final.pdb', map_p2c[p],resolutions=[1,10], color=0.1,offset=0,attachbeads=True,missingbeadsize=beadsize)
...
for p in map_p2c:
    simo.set_coordinates_from_rmf(p, "test_rmfs/"+rmf_file,0, rmf_component_name=None,check_number_particles=False)
...
simo.set_coordinates_from_rmf("rnaribo", "test_rmfs/"+rmf_file,0, rmf_component_name=None)
...
simo.set_coordinates_from_rmf("eIF1", "test_rmfs/"+rmf_file,0, rmf_component_name=None)
